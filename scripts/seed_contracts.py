...
SEED = {
    "Silicon Valley Bank": dict(
        ...
        country="US",
        fn_id="F-BANK-01",
        fn="Deposit & treasury services",
        svc="banking infrastructure",
        law="US",
        data=["US", "IE"],
        proc=["US"],
        subs=[
            {"name": "AWS", "rank": 1, "country": "US"},
            {"name": "Verapoint", "rank": 2, "country": "IE"},
        ],
        subst=SubstitutabilityRating.NOT_SUBSTITUTABLE,
        exit_plan=False,
        cost=1_200_000,
    ),
    "Stripe": dict(
        ...
        subst=SubstitutabilityRating.HIGHLY_COMPLEX,
        exit_plan=True,
        cost=450_000,
    ),
    "Fifth Third Bancorp": dict(
        ...
        country="US",
        fn_id="F-BANK-02",
        fn="Correspondent banking",
        svc="banking infrastructure",
        law="US",
        data=["US"],
        proc=["US"],
        subs=[{"name": "Azure", "rank": 1, "country": "US"}],
        subst=SubstitutabilityRating.MEDIUM_COMPLEX,
        exit_plan=True,
        cost=300_000,
    ),
}
...
async def main() -> int:
    async with SessionFactory() as s:
        ...
        for name, cfg in SEED.items():
            v = (
                await s.execute(select(Vendor).where(Vendor.display_name == name))
            ).scalar_one_or_none()
            if v is None:
                print(f"  skip {name} — vendor not found")
                continue
            existing = (
                await s.execute(select(Contract).where(Contract.vendor_id == v.id))
            ).scalar_one_or_none()
            if existing:
